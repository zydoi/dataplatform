...
public class BookService {
	...
	public List<BookDTO> findAll(int start, int size, String direction,
			String... orders) {
		...
		if (orders == null || orders.length == 0) {
			...
			books = bookDao.findAll(
					new PageRequest(start, size, new Sort(d, orders)))
					.getContent();
		}
	...
	@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	public List<BookDTO> findBooksByName(String name, int start, int size,
			String direction, String... orders) {
		List<Book> books = null;
		if (orders == null || orders.length == 0) {
	...
	public BookDetailsDTO findByIsbn(String isbn) {
		...
		// get translators
		dto.setTranslators(DTOAssembler.assembleAuthorDTOs(book
				.getTranslators()));
		// get tags
		dto.setTags(DTOAssembler.assembleTagDTOs(book.getAssociatedTags()));
		// get recommendations
		if (dto.getTags().size() != 0) {
			List<BookDTO> recommendations = DTOAssembler
					.assembleBookDTOs(bookDao
							.findByAssociatedTagsTagName(dto.getTags().get(0)
									.getName(), new PageRequest(0, 10)));
	...
	@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	public List<BookDTO> findByTagName(String tag, int start, int size,
			String direction, String... orders) {
		List<Book> books = null;
		if (orders == null || orders.length == 0) {
			books = bookDao.findByAssociatedTagsTagName(tag, new PageRequest(
					start, size));
		} else {
			Direction d = Direction.fromString(direction);
			books = bookDao.findByAssociatedTagsTagName(tag, new PageRequest(
					start, size, new Sort(d, orders)));
		}
		return convertToBookDTOs(books);
	}

	@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	public List<BookDTO> findSaleBooks(int start, int size, String direction,
			String... orders) {
		List<Book> books = null;
		if (orders == null || orders.length == 0) {
	...
	@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	public List<BookDTO> findPromotionBooks(int start, int size,
			String direction, String... orders) {
		List<Book> books = null;
		if (orders == null || orders.length == 0) {
	...
	@Transactional(readOnly = true, propagation = Propagation.REQUIRED)
	public List<BookDTO> findNewBooks(int start, int size, String direction,
			String... orders) {
		List<Book> books = null;
		if (orders == null || orders.length == 0) {
	...
	public List<BookDTO> findLikedBooks(String userName, int start, int size) {
		StringBuilder sb = new StringBuilder(256);
		sb.append("select l from User u join u.likeBooks l ").append(
				"where u.name = :name");
		Query q = entityManager.createQuery(sb.toString());
		q.setParameter("name", userName);
		q.setFirstResult(start * size);
	...
	public List<BookDTO> findSearchedBooks(String userName, int start, int size) {
		StringBuilder sb = new StringBuilder(256);
		sb.append("select s from User u join u.searchBooks s ").append(
				"where u.name = :name");
		Query q = entityManager.createQuery(sb.toString());
		q.setParameter("name", userName);
		q.setFirstResult(start * size);
